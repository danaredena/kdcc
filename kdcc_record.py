...
def fill():
    try:
        ...
        session = DBSession()
        new_faculty = Students(nickname='Diane', first_name='Diane', middle_name='Yap', last_name='Red', suffix='Jr.', address='Tandang Sora, Quezon City', birth_date='11/07/2010', sex='F', date_of_admission='02/01/2017',guardian1_name='Scarlett Red', contact_number1='09431234567', up_dependent='yes')
        session.add(new_faculty)
        session.commit()

        #3: ADD FACULTY (arbitrary) 'Bob'
        session = DBSession()
        new_student = Faculty(id_number = "001", first_name='Bob', middle_name='Dash', last_name='Parr', suffix='III', address='pixar', birth_date='04/09/1973', sex='M', date_of_employment='03/07/2015', position='Rogue', monthly_rate=15000, contact_number='09761234567', pers_tin='9302', pers_ssn='93043', pers_philhealth='340393', pers_accntnum='3094920')
        session.add(new_student)
        session.commit()

        #4: ADD STUDENT (arbitrary) 'Ohm'
        session = DBSession()
        new_student = Students(nickname='Ohm', first_name='Ohm', middle_name='My', last_name='Gulay',  suffix='III', address='Tandang Sora, Quezon City', birth_date='11/07/2008', sex='F', date_of_admission='02/07/2017', guardian1_name='Heidi Klum', contact_number1='09431299997', up_dependent='no')
        session.add(new_student)
        session.commit()

        #5: ADD FACULTY (arbitrary) 'Lulu'
        session = DBSession()
        new_faculty = Faculty(id_number = "002", first_name='Lulu', middle_name='Lala', last_name='Lang',  suffix='Sr.', address='Quezon City', birth_date='04/09/1873', sex='M', date_of_employment='06/07/2015', position='Cook', monthly_rate=10000, contact_number='09766777567', pers_tin='9542', pers_ssn='9365', pers_philhealth='64640393', pers_accntnum='3640')
        session.add(new_faculty)
        session.commit()
        ...
        session = DBSession()
        session.query(MonthlyCutoff).delete()
        session.commit()
